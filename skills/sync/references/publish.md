...
# .mifune/skills/ with .claude/skills as a symlink; older non-relocated targets
...
```bash
bash .mifune/skills/eval/run.sh 2>&1 | grep -E 'REGRESSION|PASS|FAIL'
```
...
Note: current development uses the relocated `.mifune/skills` layout with
