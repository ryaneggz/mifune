...
### 2. `.mifune/skills` symlink relocation

Current development on both remotes uses `.mifune/skills` as the shared
skill source of truth plus back-compat symlinks such as
`.claude/skills → .mifune/skills`. Older upstream history predates the
...
- **During catchup:** if an upstream commit edits `.claude/skills/<x>/SKILL.md`,
  ...
  `.mifune/skills/<x>/SKILL.md` automatically — no manual retarget needed.
  Verify the index keeps only the `120000` symlink blob.
- **During publish:** preserve the target branch's current layout. On the
  current relocated layout, sanitize and validate `.mifune/skills/**` while
...
- **Eval runner path follows the branch layout:** on the current relocated
  layout use `bash .mifune/skills/eval/run.sh`; on older non-relocated
