...
| **Monitored async loop** *(default build executor)* | **The default executor for `/spec` / `/ship-spec` builds** — `scripts/ralph.sh <slug>` runs unattended as a detached loop until its `STATUS: COMPLETE` sentinel | Advisor launches the loop in a named tmux session and **owns the terminal watch** itself — a background poll on the executor's `STATUS:` sentinel (`COMPLETE` / `SESSION-GONE` / max-iters), **never delegated to a sub-agent** (a `run_in_background` sub-agent Advisor returns early instead of staying alive to finalize). For **multiple independent tasks** the Advisor runs **one ralph loop per task in parallel** (each its own slug + named tmux session) and watches each sentinel concurrently. `/delegate` is an **optional within-iteration** fan-out tool a ralph iteration *may* use for one story's disjoint files — it **never replaces the loop** (`--executor=delegate-advisor` selects the legacy `/delegate`-orchestrated build instead). The loop **surfaces blocks** back rather than shipping them; on a block the Advisor diagnoses, corrects the task's acceptance criteria, and relaunches. The loop stops at the sentinel — the Advisor **finalizes through the promotable gate** (draft → `/pr-audit` → ready) and never lets the loop open a ready PR. See `scripts/ralph.sh` and `.claude/skills/ship-spec/SKILL.md` Stage 10 for the done-sentinel, single-owner-handoff, and honest-exit invariants this variant preserves. |
