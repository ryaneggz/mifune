...
- `.mifune/skills/weigh/scripts/score-trajectories.mjs` — the harness-owned deterministic-first weight function (frozen `DEFAULT_WEIGHTS`, sum 100).
- `.mifune/skills/rlm/scripts/query-context.mjs` — the context-addressing primitive (grep/slice/chunk-map with a max-bytes guard).
