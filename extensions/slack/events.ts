// Cron-driven scheduler that watches a directory for event JSON files and dispatches
// synthetic Slack events. Ported near-verbatim from
// packages/slack/src/events.ts (portfolio-advisor vendored copy).
...
export type HarnessEvent = ImmediateEvent | OneShotEvent | PeriodicEvent;

// ============================================================================
// Parsing & synthetic event helpers (exported for testing / harness tooling)
// ============================================================================

export function parseEventContent(content: string, filename: string): HarnessEvent | null {
...
export function buildSyntheticEvent(filename: string, event: HarnessEvent): SlackEvent {
...
export class EventsWatcher {
	...
	constructor(
		...
		private dispatch: (event: SlackEvent) => Promise<void>,
	) {
		this.startTime = Date.now();
	}

	/**
	 * Start watching for events. Call this after the bot is ready.
	 */
	start(): void {
	...
	private async handleFile(filename: string): Promise<void> {
		...
		// Parse with retries
		let event: HarnessEvent | null = null;
		let lastError: Error | null = null;

		for (let i = 0; i < MAX_RETRIES; i++) {
			try {
				const content = await readFile(filePath, "utf-8");
				event = parseEventContent(content, filename);
				break;
			} catch (err) {
	...
	private execute(filename: string, event: HarnessEvent, deleteAfter: boolean = true): void {
		const syntheticEvent = buildSyntheticEvent(filename, event);

		// Dispatch the synthetic event
		const enqueued = this.dispatchEvent(syntheticEvent);

		if (enqueued && deleteAfter) {
			// Delete file after successful dispatch (immediate and one-shot)
			this.deleteFile(filename);
		} else if (!enqueued) {
			log.logWarning(`Event dispatch failed, discarded: ${filename}`);
	...
	private dispatchEvent(event: SlackEvent): boolean {
		try {
			// Fire-and-forget; errors are caught inside dispatch callback
			this.dispatch(event).catch((err) => {
				log.logWarning("Event dispatch error", err instanceof Error ? err.message : String(err));
			});
			return true;
		} catch (err) {
			log.logWarning("Event dispatch threw synchronously", err instanceof Error ? err.message : String(err));
			return false;
		}
	}
...
/**
 * Create and start an events watcher. The watch path defaults to
 * `${baseDir}/events/` — pass baseDir (same as the store's baseDir).
 */
export function createEventsWatcher(
	baseDir: string,
	dispatch: (event: SlackEvent) => Promise<void>,
): EventsWatcher {
	const eventsDir = join(baseDir, "events");
	return new EventsWatcher(eventsDir, dispatch);
}
