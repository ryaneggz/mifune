// Socket Mode client, Slack Web API wrappers, per-channel queue, and user/channel caching for the Slack extension.
...
export class SlackBot {
	...
	private eventHandler: (event: SlackEvent) => Promise<void>;
	...
	constructor(
		eventHandler: (event: SlackEvent) => Promise<void>,
	...
	enqueueEvent(event: SlackEvent): boolean {
		...
		queue.enqueue(() => this.eventHandler(event));
		return true;
	}
	...
	private setupEventHandlers(): void {
		...
		this.socketClient.on("app_mention", ({ event, ack }) => {
			...
			this.getQueue(e.channel).enqueue(() => this.eventHandler(slackEvent));
	...
	private async backfillChannel(channelId: string): Promise<number> {
		...
		// Filter: include bot's messages, exclude other bots, skip already logged
		...
		for (const msg of relevantMessages) {
			...
			this.logToFile(channelId, {
				...
				ts: msg.ts!,
				user: isBotMessage ? "bot" : msg.user!,
				userName: isBotMessage ? undefined : user?.userName,
				displayName: isBotMessage ? undefined : user?.displayName,
				text,
				attachments,
				isBot: isBotMessage,
			});
	...
	private async backfillAllChannels(): Promise<void> {
		...
		// Only backfill channels that already have a log.jsonl (bot has interacted with them before)
