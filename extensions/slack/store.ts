// Per-channel JSONL persistence, dedup tracking, and background attachment downloads for the Slack extension.
...
export interface Attachment {
	...
	local: string; // path relative to baseDir (e.g., "C12345/attachments/1732531234567_file.png")
}
...
export interface ChannelStoreConfig {
	baseDir: string; // Caller-supplied root; defaults to ~/.pi/cache/slack/ in index.ts
...
export class ChannelStore {
	...
	getChannelDir(channelId: string): string {
		const dir = join(this.baseDir, channelId);
		if (!existsSync(dir)) {
	...
	getLastTimestamp(channelId: string): string | null {
		const logPath = join(this.baseDir, channelId, "log.jsonl");
	...
	private async downloadAttachment(localPath: string, url: string): Promise<void> {
		const filePath = join(this.baseDir, localPath);

		// Ensure directory exists
		const dir = join(this.baseDir, localPath.substring(0, localPath.lastIndexOf("/")));
		if (!existsSync(dir)) {
			mkdirSync(dir, { recursive: true });
		}
